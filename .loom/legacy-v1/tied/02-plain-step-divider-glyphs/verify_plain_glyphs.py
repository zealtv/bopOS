...
def check_names_and_glyphs(page, width):
    ...
    # Superseded by 24-show-divider-and-glyph-repass/02 (Bob, 2026-07-21:
    # "remove the added iconography for step and divider, it's messy. Make
    # step just '+', and divider '/'"). The drawn-SVG assertions this stitch
    # shipped are inverted to their single-character replacements rather than
    # left permanently red.
    check(f"{width}px: add-step glyph is a single character, no svg",
          bool(step and not step["hasSvg"] and step["text"] == "✛"), repr(step))
    check(f"{width}px: add-divider glyph is a single character, no svg",
          bool(div and not div["hasSvg"] and div["text"] == "╱"), repr(div))
    check(f"{width}px: the two glyphs are distinct",
          bool(step and div and step["text"] and div["text"] and step["text"] != div["text"]),
          repr((step and step["text"], div and div["text"])))
    ...
    check(f"{width}px: glyph slot back to a single-character 14px",
          bool(step and step["glyphWidth"] == "14px"), repr(step and step["glyphWidth"]))
    return info
