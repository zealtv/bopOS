#!/usr/bin/env python3
"""Screenshot harness for the hierarchy accordions (01/5).

Carried over from `4-row-regrind`'s `shoot_control_panel.py` with one shot
added: the Control tab with the `filter` branch collapsed, which is the whole
subject of this stitch.
...
themes:
  ...
  * the Control tab's iframe surface (#dashboard-live-view), branch open
  * the same, branch collapsed
  * the Device tab's control panel (#device-control)

Usage: shoot_accordion.py <output-dir>
"""
...
def shoot(page, out_dir, theme):
    ...
    click_in_frame("[data-gen-toggle]")
    ...
    # --- the accordion collapsed (01/5) ---
    # `<details>` toggles itself, so clicking the summary is the whole gesture;
    # the collapse is then written to localStorage and survives the heartbeat.
    click_in_frame('.live-param-branch[data-param-branch="filter"] > summary')
    time.sleep(.6)
    page.locator("#dashboard-live-view").screenshot(
        path=os.path.join(out_dir, f"control-tab-collapsed-{theme}.png"))
    click_in_frame('.live-param-branch[data-param-branch="filter"] > summary')
