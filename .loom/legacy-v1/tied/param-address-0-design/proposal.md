...
Status: **ratified by Bob on 2026-07-15**, including the companion Seat-group
cross-impact review. No implementation or OSC contract change has been made.
Bob explicitly requires the true nested OSC address to survive the engine
boundary for every engine; the earlier Pd-oriented dot-flattening proposal is
rejected and superseded here.
...
This proposal works with `all` or one numeric Seat ID today and with the
companion lowercase `g<group-id>` selector if that design is ratified. Selection
scope never collapses or reinterprets the parameter path:

```text
/g1/p/track1/fx/distortion  ->  /p/track1/fx/distortion
```

`g1` belongs only to fleet routing. `track1/fx/distortion` remains the canonical
parameter identity and the engine-visible OSC hierarchy.
...
- UDP ports, `/os/params`, heartbeat, sync, cue, point, distribution, or
  assignment wire shapes;
- selector matching for nested addresses alone; lowercase `g<id>` matching and
  persisted membership belong to the companion Seat-group implementation;
...
9. the cross-product of flat/nested parameter paths and `all`/Seat/group
   selectors strips exactly one selector segment and preserves the parameter
   hierarchy byte-for-spelling.
...
The parameter contract/model/relay foundation should land before the companion
Seat-group matcher and UI integration. Group work can then reuse variable-length
`/p/...` address construction instead of touching engine delivery twice.
...
## Seat-group cross-impact review

Bob's companion Seat-group proposal uses the lowercase selector `g<id>`, for
example:
...
The survey found no collision with nested parameter identity. Selector and
parameter hierarchy occupy separate address regions and should remain separate
helpers in code and state:

- `selector_matches(selector, seat_id, groups)` owns only `all`, numeric Seat,
  and group targeting;
- manifest qualification owns only `path + name`;
- dashboard/preset keys never include `g1`;
- engines never receive `g1`;
- group-targeted parameter writes update every member Seat, including offline
  and unbound Seats, so later catch-up uses the same canonical parameter key.

The full membership/persistence design is in the lore item
`2026-07-15-seat-group-selector-design`. It recommends node-local persisted
membership and `/all/os/groups <uid> ...`; those mechanics do not change this
manifest or engine-surface proposal.

## Ratified decisions

Bob ratified this proposal and the companion Seat-group proposal together on
2026-07-15. The binding choices are:

1. `path` is a separate array plus leaf `name`; slash-separated names are not
   accepted as manifest structure.
2. Dashboard and preset state use canonical slash-joined keys, with path moves
   treated as remove plus add.
3. `path` is authoritative hierarchy; legacy `group` remains flat presentation
   compatibility and never migrates implicitly into the wire path.
4. Lowercase `g<id>` selection composes with nested parameters exactly as shown
   and is included in the combined verification matrix.
