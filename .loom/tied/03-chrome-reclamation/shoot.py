        ...
        try:
            ...
            with sync_playwright() as pw:
                ...
                for theme in ("light", "dark"):
                    # Set the theme from an init script, not a post-load
                    # evaluate: the Control tab hosts the panel in an IFRAME,
                    # whose document read its own theme at parse time. A
                    # post-load evaluate on the parent never reached it, so
                    # every "dark" app-control shot came out with a light
                    # panel inside a dark app (visible in
                    # 02-token-promotion's before-app-control-*-dark.png).
                    # NOTE the key: theme.js reads `bopos-theme`. The old
                    # harness wrote `bopos.theme`, which nothing reads — the
                    # parent only looked themed because the same evaluate also
                    # set documentElement.dataset.theme by hand.
                    init = ("try { localStorage.setItem('bopos-theme', '%s'); }"
                            " catch (e) {}" % theme)
