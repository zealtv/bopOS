...
- [x] **Listener heading as drag-dial:** the little circle on the heading
      line becomes the control — click-drag it like a dial. Remove the
      listener-heading number box from the UI.
- [x] **Device/element dot opacity:** stop modulating dot opacity by point
...
- [x] **Point size/speed edits must not move the point:** changing size or
      speed currently disturbs position/trajectory — fix so those are
      orthogonal.
- [x] **Clip points to the space:** points currently draw past the bounds of
      the space; obscure them beyond the bounds (SVG clip path).
- [x] **Points list:** a list of points so one can be selected without
      clicking it on the map (moving points are hard to hit). Selection from
      the list behaves exactly like clicking the point.
- [x] Playwright `verify_*.py`: heading drag changes heading (mind the drag
