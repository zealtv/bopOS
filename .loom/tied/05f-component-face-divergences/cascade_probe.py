...
can be diffed across a selector rewrite.
    ...
    python cascade_probe.py --doc dashboard   > before-dashboard.json
    python cascade_probe.py --doc facilitator > before-facilitator.json
    python cascade_probe.py --diff before.json after.json [--allow-host <host>]

`--doc` selects which document's stylesheet set to load. 05c only ever measured
the dashboard's cascade; the facilitator loads `facilitator.css` INSTEAD of
`style.css`, so its base layer for the drawer was a different one and went
unmeasured. Stylesheets are read from the working tree, so stash/apply or
git-stash between the two runs rather than passing paths.
...
KINDS = ["lfo", "fade", "loop"]
# The document's own base stylesheet, then the shared layers in load order.
DOCS = {
...
def diff(before_path, after_path, allow_hosts=("bare-div",)):
    ...
    for context in sorted(before):
        ...
        if not context.startswith(tuple(allow_hosts)) and changed:
...
if __name__ == "__main__":
    if sys.argv[1] == "--diff":
        allow = ["bare-div"]
        if "--allow-host" in sys.argv:
            allow.append(sys.argv[sys.argv.index("--allow-host") + 1])
        diff(sys.argv[2], sys.argv[3], tuple(allow))
    elif sys.argv[1] == "--doc":
