...
- [ ] **Schema**: `installation.json` gains `seats` (keyed by id-as-string:
...
- [ ] **Hard break — no migration** (Bob §7 ruling): the new shape replaces
      the old; old `installation.json`/venue files are not honoured. Add a
      `schema` version field so *future* changes can migrate.
- [ ] **ws surface**: seat CRUD (`add_seat`, `update_seat` — name/positions/
