...
thread: existing `bopos.out~` spatializes each real engine before the OS stereo sum,
...
reuse the fixed-stereo semantics established by `bopos.out~`, rather than
creating a competing preview protocol. Multichannel design belongs here only
after a concrete DAW/stem need exists.
