...
still unverified and Bob's composition machine is a Mac — build Linux-first,
and keep the fan-out-relay fallback (bind 6660 once, re-send to per-instance
localhost ports via startup message) as a documented seam, not an
implementation, until the macOS run answers.
