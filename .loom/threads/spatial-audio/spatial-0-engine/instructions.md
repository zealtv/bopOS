...
- [ ] Engine in the dashboard backend: a point (x, y) + radius + falloff curve;
      animatable (programmatic path/LFO now, drag arrives with spatial-1).
- [ ] Per-device gain from `installation.json` positions, sent at ~20–30 Hz.
...
- [ ] Compose with the existing gain model, don't fight it: facilitator already
...
- [ ] simfleet: nothing new needed if it already logs `/p/gain`; verify it does.
- [ ] verify_*.py: animate a path across sim positions, assert per-device gain
