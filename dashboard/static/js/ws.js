(function () {
  class ReconnectingSocket {
    constructor(path) { this.path = path; this.handlers = {}; this.delay = 500; this.connect(); }
    connect() {
    ...
    on(type, callback) { (this.handlers[type] ||= []).push(callback); }
    emit(type, data) { (this.handlers[type] || []).forEach(callback => callback(data)); }
