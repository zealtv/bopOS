...
5. `ui-tabs/tabs-0-ia-proposal` — short Overview / Spatial / Fleet management /
   Patch editor information architecture proposal.
6. Bob ratifies the tabs-0 information architecture.
7. `ui-tabs/tabs-1-skeleton` — implement the ratified tab structure without
   redesigning behavior.
8. `ui-tabs/tabs-2-review-session` — hands-on review with Bob inside the real
   tabbed dashboard; split and complete any resulting fixes.
9. `patch-workflow-friction/friction-0-docs` — the simple composer workflow
   guide, written against the finished editor and tabs.
10. `patch-workflow-friction/friction-1-starter-kit` — finish the starter-kit
    path and teaching copy.

If bop000 or Bob is unavailable, leave `fp-4` waiting and continue the autonomous
sequence at `pe-2`. Other hardware and co-design waits remain outside this runway
